...
bool isArmor(Armor& armor)
   {
    Light light_1 = armor.left_light;
            ...
            bool light_ratio_ok = light_length_ratio > 0.1;
            // Distance between the center of 2 lights (unit : light length)
            float avg_light_length = (light_1.length + light_2.length) / 2;
            float center_distance = norm(light_1.center - light_2.center) / avg_light_length;
            bool center_distance_ok = (0.8 < center_distance&&center_distance < 2.8) ||(3.2 < center_distance&&center_distance < 4.3);
            // Angle
            Point2f diff = light_1.center - light_2.center;
            float angle = abs(atan(diff.y / diff.x)) / CV_PI * 180;
            bool angle_ok = angle < 40.0;

//            // judge angle proximity
//            float angle1 = tan(light_1.length - light_1.width);
//            float angle2 = tan(light_2.length - light_2.width);
//            bool sameAngle = angle1 - angle2 < 2 && angle1 - angle2 > -2;
            //ratio
            bool goodRatio = light_1.length / (light_2.center.x - light_1.center.x ) > 0.3 && light_1.length / (light_2.center.x - light_1.center.x ) < 3;

            bool is_armor = light_ratio_ok && center_distance_ok && angle_ok  && goodRatio;

            armor.armor_type = center_distance > 3.2 ? LARGE : SMALL;
